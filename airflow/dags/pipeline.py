...
if not is_venv_installed():
    ...
    def PPP_fraud_detection2():
        ...
        def clean_data():
            ...
            print("Storage folder path:", storage_folder)
            print("Storage folder contents:", os.listdir(storage_folder))
            print("storage path exists", os.path.exists(storage_folder))
            cleaned_csv_path = os.path.join(storage_folder, "cleaned_data.csv")
            filtered_csv_path = os.path.join(storage_folder, "filtered_data.csv")
            ...
            if os.path.exists(storage_folder):
                ...
                for csv_file in csv_files:
                    csv_file_path = os.path.join(storage_folder, csv_file)
            ...
            cleaned_concat_df = pd.concat(cleaned_dfs, ignore_index=True)
            filtered_concat_df = pd.concat(filtered_dfs, ignore_index=True)

            # Save cleaned data to CSV
            cleaned_concat_df.to_csv(cleaned_csv_path, index=False)
            print(f"Cleaned data saved to {cleaned_csv_path}")

            # Save filtered data to CSV
            filtered_concat_df.to_csv(filtered_csv_path, index=False)
            print(f"Filtered data saved to {filtered_csv_path}")

            return {"cleaned_data": cleaned_csv_path, "filtered_data": filtered_csv_path}
        ...
        def analyze_data():
            ...
            if os.path.exists(cleaned_data):
                ...
                analysis_results_path = './storage/analysis_results.csv'
        ...
        def visualize():
            """
            #### Analyze task
            Visualizes results from analyze data.
            """
            analysis_results_csv = XCom.get_one(task_ids='analyze_data')

            if analysis_results_csv:
                subprocess.run(['python', 'streamlit_app.py', analysis_results_csv])
                print("Streamlit app called successfully.")
            else:
                print("No analysis results CSV found in the data package.")
        ...
        order_data >> cleaned_data >> analyze_data >> visualize

    python_PPP_fraud_dag = PPP_fraud_detection2()
